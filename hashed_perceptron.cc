...
* This code has been slightly adapted to work with naive_sim.py, see below 
* more information by the original author
...
class HashedPerceptron {
	...
	public:
		...
		void update(uint64_t pc, uint8_t branch_type, uint8_t taken, uint8_t prediction, uint64_t target) 
		{
...
extern "C" 
{
	HashedPerceptron* create_HashedPerceptron() 
	{ 
		return new HashedPerceptron; 
	}
	
	void delete_HashedPerceptron(HashedPerceptron* bpred) 
	{
		delete bpred;
	}


	void initialize_HashedPerceptron(HashedPerceptron* bpred) 
	{
		bpred->initialize(); 
	}

	uint8_t predict_HashedPerceptron(HashedPerceptron* bpred, uint64_t ip) 
	{ 
		return bpred->predict(ip); 
	}

	void update_HashedPerceptron(	HashedPerceptron* bpred, uint64_t ip, uint8_t branch_type, uint8_t taken,
											uint8_t predictor, uint64_t target) 
	{ 
		bpred->update(ip, branch_type, taken, predictor, target); 
	}
}
